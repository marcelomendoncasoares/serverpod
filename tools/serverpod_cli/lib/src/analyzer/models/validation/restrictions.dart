...
class Restrictions {
  ...
  List<SourceSpanSeverityException> validateTableIndexName(
    ...
    if (definition is ModelClassDefinition && definition.tableName != null) {
      ...
      return [
        ..._validateDuplicateIndexShape(indexName, span),
      ];
    }
  ...
  List<SourceSpanSeverityException> validateIndexUniqueKey(
    ...
    return _validateDuplicateIndexShape(parentNodeName, span);
  }
  ...
  List<SourceSpanSeverityException> validateFieldUniqueValue(
    ...
    if (content is bool) {
      if (content) {
        return _validateAutoGeneratedUniqueIndexShapeForField(
          parentNodeName,
          span,
        );
      }
  ...
  List<SourceSpanSeverityException> validateUniquePerFieldsValue(
    ...
    var fieldErrors = _validateIndexFieldNames(perFields, span);
  ...
  List<SourceSpanSeverityException> validateIndexFieldsValue(
    ...
    var fieldErrors = _validateIndexFieldNames(convertIndexList(content), span);
    if (fieldErrors.isNotEmpty) return fieldErrors;
  ...
  List<SourceSpanSeverityException> _validateDuplicateIndexShape(
    String indexName,
    SourceSpan? span,
  ) {
    var definition = documentDefinition;
    ...
    for (var candidate in definition.indexes) {
    ...
    var duplicate = _findIndexWithSameShape(
      definition,
      index,
      excludeIndexName: indexName,
    );
    if (duplicate == null) return [];

    return [
      SourceSpanSeverityException(
        _duplicateIndexShapeMessage(duplicate),
        span,
      ),
    ];
  }
  ...
  _validateAutoGeneratedUniqueIndexShapeForField(
    String fieldName,
    SourceSpan? span,
  ) {
    var definition = documentDefinition;
    ...
    var autoIndex = field.autoGeneratedUniqueIndexDefinition(
      definition.tableName!,
      definition.fieldsIncludingInherited,
    );
    ...
    var duplicate = _findIndexWithSameShape(
      definition,
      autoIndex,
      excludeIndexName: autoIndex.name,
    );
    if (duplicate == null) return [];
  ...
  SerializableModelIndexDefinition? _findIndexWithSameShape(
    ModelClassDefinition definition,
    SerializableModelIndexDefinition index, {
    required String excludeIndexName,
  }) {
    for (var candidate in definition.indexesIncludingInherited) {
      if (candidate.name == excludeIndexName) continue;
      if (_indexesHaveSameShape(index, candidate)) return candidate;
    }

    return null;
  }

  bool _indexesHaveSameShape(
    SerializableModelIndexDefinition a,
    SerializableModelIndexDefinition b,
  ) {
    return a.type == b.type &&
        a.unique == b.unique &&
        a.ginOperatorClass == b.ginOperatorClass &&
        a.vectorDistanceFunction == b.vectorDistanceFunction &&
        const ListEquality<String>().equals(a.fields, b.fields) &&
        const MapEquality<String, String>().equals(a.parameters, b.parameters);
  }

  String _duplicateIndexShapeMessage(
    SerializableModelIndexDefinition duplicate,
  ) {
    var owner = parsedModels.findAutoUniqueIndexOwner(duplicate.name);
    if (owner != null && owner.model == documentDefinition) {
      return 'An index with the same columns and configuration already exists '
          'as "${duplicate.name}" (auto-generated for field '
          '"${owner.field.name}"). Duplicate indexes with the same shape are '
          'not allowed.';
    }

    return 'An index with the same columns and configuration already exists '
        'as "${duplicate.name}". Duplicate indexes with the same shape are '
        'not allowed.';
  }
}
