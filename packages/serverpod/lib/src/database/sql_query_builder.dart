...
class SelectQueryBuilder {
  ...
  SelectQueryBuilder withWhereRelationInResultSet(
    ...
    var strIds = ids.map(Serverpod.instance.encoder.convert).join(', ');
        ...
        .map((row) {
          var values = selectedColumns
              .map((column) {
                ...
                return Serverpod.instance.encoder.convert(
                  unformattedValue,
  ...
  return orderBy
      .mapIndexed((index, order) {
        ...
        if (column is ColumnCount) {
          ...
          if (orderDescending) {
            str += ' DESC';
            if (column.table.tableRelation != null) str += ' NULLS FIRST';
          } else {
            if (column.table.tableRelation != null) str += ' NULLS LAST';
          }
        }
