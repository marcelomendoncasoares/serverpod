  ...
  withServerpod(
    ...
    (sessionBuilder, endpoints) {
      ...
      tearDown(() async {
        ...
        await SecuredRecord.db.deleteWhere(
          session,
      ...
      test(
        'when inspecting the policies '
        'then the ownership policy exists.',
        () async {
          var result = await session.db.unsafeQuery(
            'SELECT polname FROM pg_policy '
            'JOIN pg_class ON pg_class.oid = pg_policy.polrelid '
            'WHERE relname = \'${SecuredRecord.t.tableName}\';',
          );
          var names = result.map((r) => r.toColumnMap()['polname']).toList();
          expect(
            names,
            contains(
              '${SecuredRecord.t.tableName}_${SecuredRecord.t.ownerId.columnName}_rls',
            ),
          );
        },
      );
      ...
      test(
        'when running transactionForUser '
        'then serverpod.user_id is set to the authenticated user.',
        () async {
          ...
          var value = await sessionA.db.transactionForUser((transaction) async {
      ...
      test(
        'when two users own different rows '
        'then each user only sees their own rows.',
        () async {
          var sessionA = sessionBuilder
              .copyWith(
                authentication: AuthenticationOverride.authenticationInfo(
          ...
          var visibleToA = await sessionA.db.transactionForUser(
            (transaction) async {
              // Drop to a non-superuser role so row-level security is enforced.
              await sessionA.db.unsafeQuery(
                'SET LOCAL ROLE $rlsRole',
                transaction: transaction,
              );
              return SecuredRecord.db.find(sessionA, transaction: transaction);
            },
          );
          var visibleToB = await sessionB.db.transactionForUser(
            (transaction) async {
              await sessionB.db.unsafeQuery(
                'SET LOCAL ROLE $rlsRole',
                transaction: transaction,
              );
              return SecuredRecord.db.find(sessionB, transaction: transaction);
            },
          );

          expect(
            visibleToA.map((r) => r.name),
            unorderedEquals(['a-1', 'a-2']),
          );
          expect(
      ...
      test(
        'when a user has no rows '
        'then nothing is visible even though other rows exist.',
        () async {
          var sessionA = sessionBuilder
              .copyWith(
                authentication: AuthenticationOverride.authenticationInfo(
      ...
      test(
        'when querying under the restricted role without transactionForUser '
        'then no rows are visible even though rows exist.',
        () async {
          var sessionA = sessionBuilder
              .copyWith(
                authentication: AuthenticationOverride.authenticationInfo(
          ...
          var visibleToA = await sessionA.db.transaction((transaction) async {
            await sessionA.db.unsafeQuery(
              'SET LOCAL ROLE $rlsRole',
              transaction: transaction,
            );
