...
void main() async {
  ...
  group('Given vector column in database', () {
    ...
    test(
        'when filtering using closerThan with L2 distance then matching rows are returned.',
        () async {
      var result = await Types.db.find(
        session,
    ...
    test(
        'when filtering using closerThan with cosine distance then matching rows are returned.',
        () async {
      var result = await Types.db.find(
        session,
    ...
    test(
        'when filtering using closerThan with inner product distance then matching rows are returned.',
        () async {
      var result = await Types.db.find(
        session,
    ...
    test(
        'when filtering using closerThan with L1 distance then matching rows are returned.',
        () async {
      var result = await Types.db.find(
        session,
