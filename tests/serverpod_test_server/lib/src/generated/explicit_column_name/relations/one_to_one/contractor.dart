...
class ContractorRepository {
  ...
  /// Emits [Contractor]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `Contractor.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<Contractor>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<ContractorTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<Contractor>(
      where: where?.call(Contractor.t),
      orderBy: orderBy?.call(Contractor.t),
      orderByList: orderByList?.call(Contractor.t),
      limit: limit,
