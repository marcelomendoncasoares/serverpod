...
class DecimalDefaultPersistRepository {
  ...
  /// Upserts all [DecimalDefaultPersist]s in the list and returns the resulting rows.
  ///
  ...
  Future<List<DecimalDefaultPersist>> upsert(
    _i1.DatabaseSession session,
    List<DecimalDefaultPersist> rows, {
    required _i1.ColumnSelections<DecimalDefaultPersistTable> conflictColumns,
    _i1.ColumnSelections<DecimalDefaultPersistTable>? updateColumns,
    _i1.WhereExpressionBuilder<DecimalDefaultPersistTable>? updateWhere,
    _i1.Transaction? transaction,
  }) async {
    return session.db.upsert<DecimalDefaultPersist>(
      rows,
      conflictColumns: conflictColumns(DecimalDefaultPersist.t),
      updateColumns: updateColumns?.call(DecimalDefaultPersist.t),
      updateWhere: updateWhere?.call(DecimalDefaultPersist.t),
      transaction: transaction,
    );
  }

  /// Upserts a single [DecimalDefaultPersist] and returns the resulting row.
  ///
  ...
  ///
  /// The returned [DecimalDefaultPersist] will have its `id` field set.
  Future<DecimalDefaultPersist?> upsertRow(
    _i1.DatabaseSession session,
    DecimalDefaultPersist row, {
    required _i1.ColumnSelections<DecimalDefaultPersistTable> conflictColumns,
    _i1.ColumnSelections<DecimalDefaultPersistTable>? updateColumns,
    ...
    return session.db.upsertRow<DecimalDefaultPersist>(
      row,
      conflictColumns: conflictColumns(DecimalDefaultPersist.t),
      updateColumns: updateColumns?.call(DecimalDefaultPersist.t),
      updateWhere: updateWhere?.call(DecimalDefaultPersist.t),
