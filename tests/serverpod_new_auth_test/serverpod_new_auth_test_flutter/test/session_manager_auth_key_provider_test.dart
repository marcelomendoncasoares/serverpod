...
void main() {
  ...
  late AuthSuccess sessionAuthSuccess;
  ...
  setUp(() async {
    ...
    sessionAuthSuccess = await client.sessionTest.createSession(testUserId);
  });
  ...
  group('Given a ClientAuthSessionManager with session auth info available',
      () {
    setUp(() async {
      await client.auth.updateSignedInUser(sessionAuthSuccess);
    });
    ...
    test(
      ...
      expect(result, 'Bearer ${sessionAuthSuccess.token}');
    });
  ...
  group('Given a ClientAuthSessionManager with custom provider delegates', () {
    ...
    late SasAuthKeyProvider customSessionProvider;
    ...
    setUp(() {
      ...
      customSessionProvider = SasAuthKeyProvider(
        getAuthInfo: () async => sessionAuthSuccess,
      );
