...
void main() {
  ...
  group('Given a `ClientAuthSessionManager` created with empty storage', () {
    setUpAll(() {
  ...
  group(
      'Given a `ClientAuthSessionManager` which has been init with a previous session from storage',
      () {
    setUpAll(() async {
      client = Client('http://localhost:8080/');
      storage = TestStorage();
