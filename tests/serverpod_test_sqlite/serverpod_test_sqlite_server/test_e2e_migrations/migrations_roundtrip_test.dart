...
import 'package:serverpod_test_sqlite_server/test_util/migration_test_utils.dart';
import 'package:test/test.dart';

void main() {
  var serviceClient = Client('http://localhost:8080/');
  ...
  group('Given multiple new protocol models with table', () {
    tearDown(() async {
      await MigrationTestUtils.migrationTestCleanup(
        resetSql:
            'DROP TABLE IF EXISTS migrated_table; DROP TABLE IF EXISTS migrated_table_2; DROP TABLE IF EXISTS migrated_table_3;',
        serviceClient: serviceClient,
      );
    });
  ...
  group(
    ...
    () {
      ...
      tearDown(() async {
        await MigrationTestUtils.migrationTestCleanup(
          resetSql: 'DROP TABLE IF EXISTS $oldTable; DROP TABLE IF EXISTS $newTable;',
          serviceClient: serviceClient,
        );
      });
