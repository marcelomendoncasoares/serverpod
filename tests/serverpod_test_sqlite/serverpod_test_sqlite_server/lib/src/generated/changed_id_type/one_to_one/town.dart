...
class TownIntRepository {
  ...
  /// Emits [TownInt]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `TownInt.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<TownInt>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<TownIntTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<TownInt>(
      where: where?.call(TownInt.t),
      orderBy: orderBy?.call(TownInt.t),
      orderByList: orderByList?.call(TownInt.t),
      limit: limit,
