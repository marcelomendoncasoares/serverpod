...
class CustomerIntRepository {
  ...
  /// Emits [CustomerInt]s matching the given query parameters every time the
  ...
  /// to that set. Pass [Table] instances such as `CustomerInt.t`.
  ///
  ...
  /// });
  /// ```
  _ida.Stream<List<CustomerInt>> watch(
    _is.DatabaseSession session, {
    _is.WhereExpressionBuilder<CustomerIntTable>? where,
    int? limit,
    int? offset,
    ...
    return session.db.watch<CustomerInt>(
      where: where?.call(CustomerInt.t),
      orderBy: orderBy?.call(CustomerInt.t),
      orderByList: orderByList?.call(CustomerInt.t),
      limit: limit,
