...
class DecimalDefaultModelRepository {
  ...
  /// Upserts all [DecimalDefaultModel]s in the list and returns the resulting rows.
  ///
  ...
  Future<List<DecimalDefaultModel>> upsert(
    _i1.DatabaseSession session,
    List<DecimalDefaultModel> rows, {
    required _i1.ColumnSelections<DecimalDefaultModelTable> conflictColumns,
    _i1.ColumnSelections<DecimalDefaultModelTable>? updateColumns,
    _i1.WhereExpressionBuilder<DecimalDefaultModelTable>? updateWhere,
    _i1.Transaction? transaction,
  }) async {
    return session.db.upsert<DecimalDefaultModel>(
      rows,
      conflictColumns: conflictColumns(DecimalDefaultModel.t),
      updateColumns: updateColumns?.call(DecimalDefaultModel.t),
      updateWhere: updateWhere?.call(DecimalDefaultModel.t),
      transaction: transaction,
    );
  }

  /// Upserts a single [DecimalDefaultModel] and returns the resulting row.
  ///
  ...
  ///
  /// The returned [DecimalDefaultModel] will have its `id` field set.
  Future<DecimalDefaultModel?> upsertRow(
    _i1.DatabaseSession session,
    DecimalDefaultModel row, {
    required _i1.ColumnSelections<DecimalDefaultModelTable> conflictColumns,
    _i1.ColumnSelections<DecimalDefaultModelTable>? updateColumns,
    ...
    return session.db.upsertRow<DecimalDefaultModel>(
      row,
      conflictColumns: conflictColumns(DecimalDefaultModel.t),
      updateColumns: updateColumns?.call(DecimalDefaultModel.t),
      updateWhere: updateWhere?.call(DecimalDefaultModel.t),
